...
import { WAVE_RESET } from '../reset/effect.js';
import { WATER_CAUSTIC_FORMAT, WATER_FIELD_FORMAT, WATER_FIELD_RESOLUTION, waterCausticShader, waterFieldShader, waterOverlayShader } from './water-reset.js';
...
export const GLINT_FORMAT = 'r16float';
export class SandPostProcess {
    ...
    waterCausticModule;
    overlayModule;
    legacyEncodePipeline;
    postPipeline;
    waterFieldPipeline;
    waterCausticPipeline;
    overlayPipeline;
    legacyEncodeGroup;
    postGroup;
    waterFieldGroup;
    ...
    compositeSceneView;
    waterSurfaceField;
    waterSurfaceFieldView;
    waterMotionField;
    waterMotionFieldView;
    waterCausticField;
    waterCausticFieldView;
    waveActive = false;
    width = 0;
    height = 0;
    device;
    legacyFormat;
    targetFormat;
    constructor(device, legacyFormat, targetFormat){
        this.device = device;
        this.legacyFormat = legacyFormat;
        this.targetFormat = targetFormat;
        this.uniform = device.createBuffer({
            label: 'Filmic post uniform',
            size: 16,
            usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
        });
        this.overlayUniform = device.createBuffer({
            label: 'Wave overlay uniform',
            size: 112,
            usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
        });
        this.sampler = device.createSampler({
            label: 'Filmic post sampler',
            magFilter: 'linear',
            minFilter: 'linear',
            mipmapFilter: 'linear',
        ...
        this.waterCausticField = device.createTexture({
            label: 'Reset water caustic field',
            size: [
                WATER_FIELD_RESOLUTION,
                WATER_FIELD_RESOLUTION
            ],
            format: WATER_CAUSTIC_FORMAT,
            usage: GPUTextureUsage.RENDER_ATTACHMENT | GPUTextureUsage.TEXTURE_BINDING
        });
        this.waterCausticFieldView = this.waterCausticField.createView();
        this.legacyEncodeModule = device.createShaderModule({
            label: 'Legacy display encode WGSL',
            code: legacyEncodeShader
        });
        this.postModule = device.createShaderModule({
            label: 'Filmic post WGSL',
            code: postShader
        });
        this.waterFieldModule = device.createShaderModule({
            label: 'Reset water field WGSL',
            code: waterFieldShader
        });
        this.waterCausticModule = device.createShaderModule({
            label: 'Reset water caustic WGSL',
            code: waterCausticShader
        });
        this.overlayModule = device.createShaderModule({
            label: 'Wave overlay WGSL',
            code: waterOverlayShader
        });
    }
    async initialize() {
        [this.legacyEncodePipeline, this.postPipeline, this.waterFieldPipeline, this.waterCausticPipeline, this.overlayPipeline] = await Promise.all([
            this.device.createRenderPipelineAsync({
                label: 'Legacy display encode',
                layout: 'auto',
                vertex: {
                    module: this.legacyEncodeModule,
                    entryPoint: 'vertex'
                },
                fragment: {
                    module: this.legacyEncodeModule,
            ...
            this.device.createRenderPipelineAsync({
                label: 'Filmic post process',
                layout: 'auto',
                vertex: {
                    module: this.postModule,
                    entryPoint: 'vertex'
                },
                fragment: {
                    module: this.postModule,
            ...
            this.device.createRenderPipelineAsync({
                label: 'Reset water field',
                layout: 'auto',
                vertex: {
                    module: this.waterFieldModule,
                    entryPoint: 'vertex'
                },
                fragment: {
                    module: this.waterFieldModule,
            ...
            this.device.createRenderPipelineAsync({
                label: 'Reset water caustics',
                layout: 'auto',
                vertex: {
                    module: this.waterCausticModule,
                    entryPoint: 'vertex'
                },
                fragment: {
                    module: this.waterCausticModule,
            ...
            this.device.createRenderPipelineAsync({
                label: 'Wave overlay composite',
                layout: 'auto',
                vertex: {
                    module: this.overlayModule,
                    entryPoint: 'vertex'
                },
                fragment: {
                    module: this.overlayModule,
        ...
        this.waterFieldGroup = this.device.createBindGroup({
            layout: this.waterFieldPipeline.getBindGroupLayout(0),
            entries: [
                {
                    binding: 0,
                    resource: {
                        buffer: this.overlayUniform
                    }
                }
            ]
        });
        this.waterCausticGroup = this.device.createBindGroup({
            layout: this.waterCausticPipeline.getBindGroupLayout(0),
            entries: [
                {
                    binding: 0,
                    resource: {
                ...
                {
                    binding: 2,
                    resource: this.waterSurfaceFieldView
                },
                {
                    binding: 3,
                    resource: this.waterMotionFieldView
                }
            ]
        });
    }
    resize(width, height) {
        ...
        this.width = width;
        this.height = height;
        this.hdrScene = this.device.createTexture({
            label: 'Linear HDR scene color',
            size: [
                width,
                height
            ],
        ...
        this.glintScene = this.device.createTexture({
            label: 'Reflective mineral HDR glints',
            size: [
                width,
                height
            ],
        ...
        this.legacyScene = this.device.createTexture({
            label: 'Legacy display scene color',
            size: [
                width,
                height
            ],
        ...
        this.compositeScene = this.device.createTexture({
            label: 'Filmic composite scene color',
            size: [
                width,
                height
            ],
        ...
        this.legacyEncodeGroup = this.device.createBindGroup({
            layout: this.legacyEncodePipeline.getBindGroupLayout(0),
            entries: [
                {
                    binding: 0,
                    resource: this.hdrSceneView
                }
            ]
        });
        ...
        this.overlayGroup = this.device.createBindGroup({
            ...
            entries: [
                ...
                {
                    binding: 2,
                    resource: this.compositeSceneView
                },
                {
                    binding: 3,
                    resource: this.waterSurfaceFieldView
                ...
                {
                    binding: 4,
                    resource: this.waterCausticFieldView
                }
            ]
        });
    ...
    setWaveState(waveState, camera) {
        ...
        const data = new Float32Array([
            ...
            ...camera.up,
            0,
            waveState.active ? 1 : 0,
            waveState.currentBaseFront,
            waveState.time,
            SAND.extent * 0.5,
            WAVE_RESET.shorelineFeather,
            WAVE_RESET.foamWidth,
            WAVE_RESET.foamTrail,
            WAVE_RESET.waterOpacity,
            WAVE_RESET.waterTintStrength,
            WAVE_RESET.washGloss,
            WAVE_RESET.rippleScale,
            WAVE_RESET.rippleDrift
        ]);
    ...
    encode(encoder, target) {
        ...
        const encodePass = encoder.beginRenderPass({
            label: 'Legacy display encode',
            colorAttachments: [
                {
                    view: this.legacySceneView,
                    loadOp: 'clear',
                    storeOp: 'store',
                    clearValue: {
                        r: 0.55,
        ...
        if (this.waveActive) {
            const fieldPass = encoder.beginRenderPass({
                label: 'Reset water fields',
                colorAttachments: [
                    {
                        view: this.waterSurfaceFieldView,
                        clearValue: {
                            r: 0,
                            g: 0,
                            b: 0,
                    ...
                    {
                        view: this.waterMotionFieldView,
                        clearValue: {
                            r: 0,
                            g: 0,
                            b: 0,
            ...
            const causticPass = encoder.beginRenderPass({
                ...
                colorAttachments: [
                    {
                        view: this.waterCausticFieldView,
                        clearValue: {
                            r: 1,
                            g: 0,
                            b: 0,
